...
## 状态（2026-07-29 · WHO IRIS + Claim Card 上游）

> 📄 **当天进展与待决事项的通读版：[`docs/STATUS_2026-07-29.md`](docs/STATUS_2026-07-29.md)**
> （含需要拍板的 5 个岔路、可自行验证的命令、以及"现在能做什么/还不能做什么"）

- ✅ **Claim Card 上游流程化（`docs/CARD_EXTRACTION_SPEC.md`）**：此前 `examples/` 下 5 张卡
  全是 `synthetic_example`——不对应任何真实论文，且代码里零抽卡逻辑。现从 225 篇带真人评审的
  论文中选 6 篇**人工拆卡**，产出**十条书面规则** + `corpus/gold/` 六张 gold 基准。
  6 篇真实论文里**没有一篇**是"单病种+单任务+人群整齐"的，而合成卡全是。
  三条最关键：主任务由**主要终点**决定（不看模型在做什么）、「不适用」≠「不匹配」、
  「论文没写」≠「我没抽出来」（后者改为硬错误）。
  **抽卡器刻意还没写**——规则必须先于它，否则它会为通过校验硬编病种/场景，且不会报错。
  schema 已吃下三条差距（`2579334`，只接入不消费），回归零副作用：`check_gates` 输出逐字
  一致、五张示例卡记录数不变。
